...
\begin{frame}[label=p1]{The problem: sorted input destroys it}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    \begin{scope}[xshift=-3.7cm]
      ...
      \node[color=avlmuted,font=\small] at (0,-3.6) {inserted 4,2,6,1,3,5,7};
      \node[color=avlfib,font=\large\bfseries] at (0,-4.3) {3 steps};
    \end{scope}
    ...
    \begin{scope}[xshift=3.6cm,yshift=0.3cm]
      ...
      % y chosen so both captions land on the same baseline as the left tree's,
      % which sits 0.3cm lower because this scope is shifted up by that much
      \node[color=avlmuted,font=\small] at (1.0,-3.9) {inserted 1,2,3,4,5,6,7};
      \node[color=avlalert,font=\large\bfseries] at (1.0,-4.6) {7 steps};
    \end{scope}
  \end{tikzpicture}
  \end{center}
...
\begin{frame}[label=p1]{The idea: check the height at every node}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[avltri,minimum size=2.4cm] (L) at (-1.7,-1.6) {};
    \node[avltri,minimum size=2.4cm] (R) at (1.7,-1.6) {};
...
\begin{frame}[label=p1]{Proof: ask the question backwards}
  \vspace{1.4em}
  \begin{center}
    {\color{avlmuted}\large Instead of ``how tall can it get?'', ask:}\\[0.9em]
    {\LARGE\color{avlfib}\bfseries What is the \emph{smallest} AVL tree
     of height $h$?}
  \end{center}
  \vspace{1.8em}
  \begin{center}
    {\color{avlmuted}If even the emptiest legal tree of height $h$ needs a lot
     of nodes,\\[0.35em]
     then a tree with few nodes \emph{cannot} be tall.}\\[1.2em]
    {\color{avlmuted}Write \fib{$N(h)$} for that smallest node count.}
  \end{center}
...
\begin{frame}[label=p1]{Each smallest tree is the two before it, under one root}
  \vspace{0.1em}
  \begin{center}
  \begin{tikzpicture}[x=1cm,y=1cm,
      ...
      miniroot/.style={circle, draw=avlfib, fill=avlfib, inner sep=0pt,
                       minimum size=4.6mm},
      medge/.style  = {draw=avlfib!75, line width=0.5pt},
      blk/.style    = {draw=avlmuted, dashed, line width=0.5pt,
                       rounded corners=2pt, inner sep=2.2pt},
      blab/.style   = {font=\scriptsize, text=avlmuted, inner sep=1pt},
      treecap/.style= {font=\small, text=avlink, align=center,
                       minimum width=2cm, minimum height=8mm}]
    % fixed extent so nothing shifts between overlays
    \path (-5.9,-4.30) rectangle (5.5,0.75);
  ...
  \vspace{-0.4em}
  \begin{center}
    \only<1>{{\color{avlmuted}The smallest legal tree of height 0 is one node.}}
    \only<2>{{\color{avlmuted}For height 1 you need a child --- and one child
              is enough, because a gap of 1 is legal.}}
    \only<3>{{\color{avlmuted}For height 2, one side must reach height 1.
              The other is as \emph{small} as the rule allows: height 0.}}
    \only<4>{{\color{avlmuted}And again: one side $T_2$, the other side $T_1$.
              \fib{The pattern does not change.}}}
    \only<5->{{\large\color{avlfib}$N(h) \;=\; N(h-1) \;+\; N(h-2) \;+\; 1$}\\[0.3em]
              {\color{avlmuted}one root, plus the two smallest trees that fit
               under it}}
  \end{center}
...
\begin{frame}[label=p1]{Where the golden ratio comes from}
  \vspace{0.3em}
  \begin{center}
    {\color{avlmuted}The $+1$ is in the way --- so absorb it.}\\[0.7em]
    $N(h) = N(h-1) + N(h-2) + 1$
    \\[0.7em]
    \onslide<2->{add 1 to both sides:\quad
      $\underbrace{N(h)+1}_{M(h)} =
       \underbrace{\big(N(h{-}1)+1\big)}_{M(h-1)} +
       \underbrace{\big(N(h{-}2)+1\big)}_{M(h-2)}$}
    \\[0.9em]
    \onslide<3->{{\large\fib{$M(h) = M(h-1) + M(h-2)$}}\quad
      {\color{avlmuted}--- Fibonacci exactly, no $+1$ left}}
  \end{center}

  \vspace{0.35em}
  \onslide<4->{
  \begin{center}
    {\color{avlmuted}Look for solutions of the form $M(h) = x^h$:}\\[0.35em]
    $x^h = x^{h-1} + x^{h-2}
  ...
  \onslide<5->{
  \begin{center}
    {\color{avlmuted}The larger root dominates:}\quad
    \fib{$\varphi = \tfrac{1+\sqrt5}{2} \approx 1.618$}\quad
    {\color{avlmuted}so}\quad \fib{$N(h) \sim \varphi^{\,h}$}
  \end{center}}
...
\begin{frame}[label=p1]{The exact count}
  \vspace{1.1em}
  \begin{center}
    {\Huge\fib{$N(h) \;=\; F(h+3) \;-\; 1$}}
  \end{center}
  \vspace{1.0em}
  \begin{center}
    {\color{avlmuted}because $M(h) = N(h)+1$ is Fibonacci, starting at
     $M(0) = 2 = F(3)$}
  \end{center}
  \vspace{0.9em}
  \onslide<2->{
  ...
  \begin{tabular}{l|cccccccc}
    {\color{avlmuted}$h$}      & 0 & 1 & 2 & 3 & 4 & 5 & 6 & \fib{\textbf{7}}\\\hline
    {\color{avlmuted}$N(h)$}   & 1 & 2 & 4 & 7 & 12 & 20 & 33 & \fib{\textbf{54}}\\
    {\color{avlmuted}$F(h{+}3)$} & 2 & 3 & 5 & 8 & 13 & 21 & 34 & \fib{55}\\
  \end{tabular}}
  \end{center}}
  ...
  \onslide<3->{
  \begin{center}
    {\color{avlmuted}and taking logs of $N(h)\sim\varphi^h$:}\quad
    {\large\fib{$h \approx \dfrac{\log_2 n}{\log_2\varphi} = 1.44\,\log_2 n$}}
  \end{center}}
...
\begin{frame}[label=p1]{Remember this number: 54}
  ...
  \begin{center}
    ...
    {\color{avlpthree}\bfseries In Part 3 we will delete one leaf from this
     exact tree --- and watch it come apart.}
  \end{center}
