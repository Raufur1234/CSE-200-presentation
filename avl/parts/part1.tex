...
% ------------------------------------------------------------------ what is a BST
\begin{frame}[label=p1]{The Classic BST}
  \vspace{0.3em}
...
\begin{frame}[label=p1]{The problem: sorted input destroys it}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.94,every node/.style={transform shape}]
    % Symmetric extent, shared with "How AVL fixes the bad case". Without it the
    % bounding box is decided by whichever side's content reaches further, the
    % \begin{center} then centres THAT, and the divider at x = 0 drifts off the
    % slide's midline -- by a different amount on each of the two frames.
    \path (-6.1,1.00) rectangle (6.1,-3.80);
...
\begin{frame}[label=p1]{The idea: check the height at every node}
  \vspace{-0.2em}
  \begin{center}
    {\avlfLead\color{avlbody}The BST got tall since one side grew and
     \emph{remained unchecked}.}
  \end{center}
  \vspace{0.6em}
  \begin{center}
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.80,every node/.style={transform shape}]
    \node[avlkey,minimum size=42\pxl] (r) at (0,0.85) {};
    ...
    \draw[avledge] (r) -- (L.north);
    \draw[avledge] (r) -- (R.north);
    % height rulers, one per side, measured from the root row to each base
    \heightbrace{-3.05}{0.55}{-2.32}{\scalebox{1.5}{height $a$}}{avlfaint}
    \heightbrace{3.05}{0.55}{-1.75}{}{avlfaint}
    \node[font=\avlfAnnot,text=avlfaint,rotate=90,anchor=north]
      at (3.15,-0.60) {\scalebox{1.5}{height $b$}};
    \draw[avlrule,line width=1\pxl,dotted] (-3.05,0.55) -- (3.05,0.55);
    \draw[avlrule,line width=1\pxl] (-2.55,-2.72) -- (2.55,-2.72);
    \node[avlannot,font=\avlfSec,anchor=north] at (0,-2.80) {one node, two subtree heights};
  \end{tikzpicture}
  \end{center}
...
\begin{frame}[label=p1]{How AVL fixes the bad case}
  % Same lattice, node sizes and verdict baseline as "sorted input destroys it"
  % two frames back, mirrored: there the legal tree was on the left, here it is
  % the answer, so it lands on the right. Each tree now carries its balance
  % factors -- all zero and green on the AVL side, one impossible -6 in red
  % with a cross on the list.
  \vspace{0.1em}
  \begin{center}
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.94,every node/.style={transform shape}]
    % same symmetric extent as frame 04, so the divider lands on the same
    % midline on both frames
    \path (-6.1,1.00) rectangle (6.1,-3.80);
    \figdivider{0}{0.95}{-3.75}

    \begin{scope}[xshift=-3.55cm,yshift=-0.15cm]
      \node[avlannot,text=avlmuted] at (0,0.95) {plain BST};
      \foreach \k [count=\i from 0] in {1,2,3,4,5,6,7} {
        \pgfmathsetmacro{\yy}{-\i*0.44}
        \pgfmathsetmacro{\xx}{-1.20+\i*0.40}
        \node[avlkeyalert,minimum size=34\pxl,font=\avlfAnnot] (c\k) at (\xx,\yy) {\k};
      ...
      \foreach \a/\b in {c1/c2,c2/c3,c3/c4,c4/c5,c5/c6,c6/c7}
        \draw[draw=avlalert,line width=1.6\pxl] (\a) -- (\b);
      % \color goes INSIDE the maths: beamer stamps its own "math text" colour
      % at the start of every $...$, so a node-level text= is ignored there.
      \node[color=avlalert,font=\avlfAnnot\wmed,anchor=west] (bflab)
        at (-0.95,0.30) {$\color{avlalert}\mathrm{bf}(1) = -6$};
      % a drawn cross, sized to the label's own x-height
      \draw[avlalert,line width=1.5\pxl,line cap=round]
        ($(bflab.east)+(0.12,-0.08)$) -- ($(bflab.east)+(0.30,0.10)$);
      \draw[avlalert,line width=1.5\pxl,line cap=round]
        ($(bflab.east)+(0.12,0.10)$) -- ($(bflab.east)+(0.30,-0.08)$);
      \node[color=avlalert,font=\avlfStmt\wbold] at (0,-3.35) {7 steps};
    \end{scope}

    \begin{scope}[xshift=3.55cm,yshift=-0.55cm]
      \node[avlannot,text=avlmuted] at (0,1.35) {AVL tree};
      \node[avlkeyok] (b4) at (0,0)        {4};
      \node[avlkeyok] (b2) at (-1.25,-1.0) {2};
      \node[avlkeyok] (b6) at (1.25,-1.0)  {6};
      \node[avlkeyok] (b1) at (-1.95,-2.0) {1};
      \node[avlkeyok] (b3) at (-0.55,-2.0) {3};
      \node[avlkeyok] (b5) at (0.55,-2.0)  {5};
      \node[avlkeyok] (b7) at (1.95,-2.0)  {7};
      \foreach \a/\b in {b4/b2,b4/b6,b2/b1,b2/b3,b6/b5,b6/b7}
        \draw[draw=avlbalance!70,line width=1.6\pxl] (\a) -- (\b);
      % every balance factor, and every one of them legal
      \node[avlannotok,anchor=west] at (0.38,0.34)
        {$\color{avlbalance}\mathrm{bf} = 0$};
      \node[avlannotok,anchor=east] at (-1.62,-0.68) {$\color{avlbalance}0$};
      \node[avlannotok,anchor=west] at (1.62,-0.68)  {$\color{avlbalance}0$};
      % leaves take their bf beside them, not beneath: underneath would crowd
      % the verdict line that has to stay level with "7 steps" opposite
      \node[avlannotok,anchor=west] at (-1.58,-2.0) {$\color{avlbalance}0$};
      \node[avlannotok,anchor=west] at (-0.18,-2.0) {$\color{avlbalance}0$};
      \node[avlannotok,anchor=west] at (0.92,-2.0)  {$\color{avlbalance}0$};
      \node[avlannotok,anchor=west] at (2.32,-2.0)  {$\color{avlbalance}0$};
      \node[color=avlbalance,font=\avlfStmt\wbold] at (0,-2.95) {3 steps};
    \end{scope}
  \end{tikzpicture}
  \end{center}
...
\begin{frame}[label=p1]{The height property}
  \vspace{0.1em}
  \begin{center}
    {\avlfLead\color{avlbody}A simple rule at each node that keeps the 
     \emph{whole} tree balanced:}\\[0.9em]
    \begin{avlpanel}[0.50\textwidth]
      {\avlfDivider\color{avlfib}$h \le 1.44 \, \log_2 n$}
    \end{avlpanel}
  \end{center}
  ...
  \vspace{0.7em}
  \begin{center}
    {\avlfSec\color{avlmuted}Never a list. Always close to the minimum possible height.}
  \end{center}
...
\begin{frame}[label=p1]{Proof: ask the question backwards}
  ...
  \begin{center}
    ...
    {\avlfStmt\color{avlink} What is the
     {\color{avlfib}\mdseries \wbold smallest} AVL tree of height $\boldsymbol{\textcolor{avlfib}{h}}$?}
  \end{center}
  ...
  \vspace{0.7em}
  \begin{center}
    {\avlfBody\color{avlbody}Even the smallest AVL tree of height $h$ needs many nodes. \\[0.3em]
    So an AVL tree with few nodes \emph{cannot} be very tall.}\\[0.9em]
    \begin{avlpanel}[0.58\textwidth]
     \fib{$N(h)$} means that smallest node count.
    \end{avlpanel}
  \end{center}
...
\begin{frame}[label=p1]{Where the golden ratio comes from}
  ...
  \vspace{0.1em}
  \begin{center}
    {\avlfSec\color{avlfaint}\avltrack{$x^2 = x+1$ has one root above 1, and it dominates}}\\[1.0em]
    ...
    \begin{avlpanel}[0.38\textwidth]
      ...
      \end{tikzpicture}
    \end{avlpanel}\\[0.9em]
    {\avlfTitle\color{avlink}$N(h) = \Theta(\varphi^{\,h})$}\\[0.9em]
    \begin{overlayarea}{\textwidth}{144\pxl}
      \centering
      \only<5>{%
        \begin{minipage}{0.74\textwidth}\centering
          {\avlfBody\color{avlbody}The smallest legal tree multiplies by
           ...
           grow like a logarithm \fib{which is why the 1.44 appears.}}
        \end{minipage}}
      \only<6>{%
        \begin{avlpanel}[0.78\textwidth]
          \panellabel{taking logs of $n \ge N(h) \ge \varphi^h$}
          {\avlfTitle\color{avlfib}$h \le \log_2 n \,/\, \log_2\varphi
            \approx 1.44 \log_2 n$}
        \end{avlpanel}}
    \end{overlayarea}
  \end{center}}
  ...
  h. On the final click, take logs: an actual tree has at least N of h nodes,
  so its height is at most log n divided by log phi. In base two, the factor
  is about 1.44. Now we know why the tree stays short. Next, count exactly.}
\end{frame}
...
%       {\avlfBody\color{avlbody}Add 1 to each minimum node count and you get
%        \fib{the Fibonacci sequence: $2,3,5,8,\ldots$}.\\[6\pxl]
%       %  At height 7, this gives \fib{$55-1=54$ nodes}.
%       }
%     \end{minipage}
%   \end{center}
...
%   exact tree and break it. The table and summary appear together: adding one
%   gives Fibonacci, and subtracting one gives our minimum node counts.}
% \end{frame}
...
%       {\color{avlpthree}In Part 3 we will delete one leaf from this exact tree, and watch it come apart.}
%     \end{avlpanelh}
%   \end{center}
