  ...
  \centering
    ...
    \only<2->{%
      \begin{avlpanel}[0.78\textwidth]
        {\avlfLead\color{avlfib}\wbold But exactly how many bad nodes do we need to rotate at?}
      \end{avlpanel}\\[0.8em]
...
\begin{frame}[label=p2]{Simulation}
  \setbeamercovered{transparent=32}
  % Was 0.5em: the tree's own bbox reserves a fixed ascent for beat 4's arc
  % (apex at y=1.30) that beats 1-3, 5, 6 never use, so on every other beat the
  % tree read as pushed down under a block of dead air. Pulled the whole
  % column block up; the tightened \path below claims back the rest.
  \vspace{-0.5em}
  \begin{columns}[T,onlytextwidth]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.84,every node/.style={transform shape}]
    ...
    % the turn arrow and its label, the bottom by the ghost of 5. Top trimmed
    % from 1.50 to 1.35: beat 4's arc apex is pivot-y (0) + radius (1.30), so
    % 1.35 is the least ascent that still clears it on every beat.
...
\begin{frame}[label=p2]{The catch}
  ...
  \begin{center}
    ...
    {\avlfStmt\wmed\color{avlink}Rotation at the first bad node and we are done?}\\[72\pxl]
