...
\newcommand{\drawfib}[3]{%
  ...
  \ifnum#1>0\relax
    ...
    % At h = 1 the right sibling is empty, so L(h-2) = 0 and the raw formula
    % would place the only child directly beneath its parent, drawing a
    % vertical stick that does not read as a tree. Flooring the sibling width
    % at one leaf slants that child down-left instead. Half a leaf separation
    % keeps it inside its own slot, so nothing collides.
    \pgfmathsetmacro{\avlRsib}{max(\avlL{#1-2},1)}%
